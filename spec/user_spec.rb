...
describe User do
  describe '#all method' do
    it 'can list all users' do
      10.times { |n| User.create(name: 'test name', email: "test#{n}@email.com", password: 'password1234') }
      user = User.all
    ...
    it 'can return the user id' do
      User.create(name: 'test name', email: 'test@email.com', password: 'password1234')
      user = User.all
  ...
  describe '#create' do
    it 'can add a user to the db' do
      User.create(name: 'Kane', email: 'Kane@email.com', password: 'Kane_password')
      user = User.all.first
  ...
  describe '#find method' do
    it 'can find user from database based on id' do
      user = User.create(name: 'Tiffany', email: 'tiffany@email.com', password: 'tiff123')
  ...
  describe '#authenticate' do
    it 'returns a user if correct username + password given' do
      user = User.create(name: 'Kane', email: 'Kane@email.com', password: 'Kane_password')
